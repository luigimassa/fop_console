...
//          @bwlab  campi custom------------
            $referente = trim(Tools::getValue('referente'));
            $azienda = trim(Tools::getValue('azienda'));
            $telefono = trim(Tools::getValue('telefono'));
            $settore = trim(Tools::getValue('settore'));
            
            //inserisco nel messaggio i dati del cliente
            $message.=PHP_EOL;
            $message .= implode(
                PHP_EOL,
                array(
...
//                );
//            }
//            elseif (!(Validate::isLoadedObject($contact)))
//            {
//                //nuovo contatto
//                $this->context->controller->errors[] = $this->trans(
//                    'Please select a subject from the list provided. ',
                ...
                if ($contact->customer_service) {
                    ...
                    if ($ct->id) {
                        ...
                        if ($lastMessage != $message || $testFileUpload) {
                            ...
                            if ($testFileUpload && rename(
                                    $file_attachment['tmp_name'],
                                    _PS_UPLOAD_DIR_ . basename($file_attachment['rename'])
                                )) {
