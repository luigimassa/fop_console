    ...
    <tfoot>
      {foreach $order.subtotals as $line}
        {if $line.type == 'shipping'}
          {hook h="displayConai" order=$order}
          {widget name='bwcliche' order=$order}
        {/if}
        {if $line.value}
      ...
      <tr class="text-xs-right line-{$order.totals.total.type}">
        <td></td>
        <td></td>
        <td class="cell-total sx">{$order.totals.total_including_tax.label}</td>
        <td class="cell-total dx">{$order.totals.total_including_tax.value}</td>
      </tr>
    </tfoot>
  </table>
</div>
