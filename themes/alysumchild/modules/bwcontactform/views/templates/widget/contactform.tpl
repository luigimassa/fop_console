...
{function name=input label="" field="" icon="" col="6" }
	{if label}
		<div class="form-group col-md-{$col}" style="display: flex; flex-direction: column;">
			<div class="icon-true relative" style="display: flex; flex-direction: row;">
	...
	{else}
		<input type="{$field->getType()}" name="{$field->getName()}" value="{$field->getValue()|escape:'htmlall':'UTF-8'}" />
	{/if}
{/function}
...
{function name=textarea label="" field="" icon=""}
	<div class="form-group col-xs-12">
		<div class="icon-true relative">
			<textarea class="form-control input-txt ac-message" {if $field->isRequired()}required="required" {/if} cols="67"
				rows="3" name="{$field->getName()}" placeholder="{$label}"></textarea>
			<svg class="svgic input-icon">
				<use xlink:href="#si-{$icon}"></use>
			</svg>
		</div>
